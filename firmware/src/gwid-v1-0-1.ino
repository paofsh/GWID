...
#define BUILDVERSION "gwid v1.0.1"
...
unsigned long wifiLastCheckMillis = 0;
const long WIFI_CHECK_INTERVAL = 5000;  // Interval to check for WiFi status (5 seconds)

uint16_t msPerUnit = 100;    // initialize the timing per unit
...
//Functions to Manage reset, WiFi, access portal, and OTA
void checkButton();
...
void setup() {
  ...
  // Configure the button pin (also external pull-up resistor)
  ...
  // Open portal if button pressed, or clear settings if nothing saved
  if (digitalRead(BUTTON_PIN) == LOW || WiFi.SSID() == "") {
    outputDebugln("Button held on startup or no credentials. Restarting to GWID_AP access portal...");
    // Turn off all pixels
    strip.clear();
    strip.show(); 
  ...
  // Attempt to connect, if failed, keep rebooting
  WiFi.mode(WIFI_STA);

  WiFi.begin(); // Connect to last saved creds
  Serial.print("Connecting to WiFi");
  int attempts = 0;
  while (WiFi.status() != WL_CONNECTED && attempts < 20) {
    delay(500);  // blocking code is okay at this point -- no WiFi connection to worry about
    Serial.print(".");
    attempts++;
  }
  Serial.println();
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    for (int i = 0; i < 11; i++) {
      digitalWrite(LED_ONBOARD, i % 2); // toggle the onboard LED
      strip.setPixelColor(RING_MIDPOINT, 255*(i % 2), 255*(i % 2), 0);
      strip.show(); 
      delay (500); // blocking code is okay at this point -- no WiFi connection to worry about
    }
    ESP.restart(); 
  ...
  if (hasSavedIP) {
    ...
    Serial.println(savedURL);
    Serial.print("Restrict_client: ");
    Serial.println(restrictClientFlag);
  }
  else {
    Serial.println("No Saved IP / Port");
  ...
  if (hasSavedIP) { 
    ...
    if (restrictClientFlag) strcpy(patternParam,"booooorooooo");
    else strcpy(patternParam,"booooobooooo");
  }
  setRingToPattern();
  strip.show();
  delay (1000);
  ...
  // If the saved IP & port were validated, Make an initial call to savedURL;
...
void loop() {
  unsigned long currentTime = millis();      // get current time for compare

  checkButton();

  if ((currentTime - wifiLastCheckMillis) >= WIFI_CHECK_INTERVAL) {
    wifiLastCheckMillis = currentTime;
    if (WiFi.status() != WL_CONNECTED) {
  ...
  if ( ((currentTime - startTime) > pattern[timingPattern][step]*msPerUnit) || flagPatternRestart ) {
    ...
    else if (strcmp(modeParam,"police") == 0) { 
      strip.setBrightness(stripBrightness);
...
void syncToSavedIP() {
  ...
  // NEOPIXEL Indicate state
  levelParam = 25;
  stripBrightness = scale100to255 (levelParam);
  strip.setBrightness(stripBrightness);
  setRingToLevel();
  if (restrictClientFlag) strcpy(patternParam,"booboorooboo");
  else strcpy(patternParam,"booboobooboo");
...
void handleGetState() {
 ...
 if (hasSavedIP) {
    if (WiFi.status() == WL_CONNECTED) { // double check connection first!
      outputDebug("Now Sending http json message: ");
      outputDebugln(jsonString);
      ...
      outputDebug("WiFi not connected. Cannot send HTTP JSON message to savedURL");
    }
  return;
 }
...
void handleModes() {   
  ...
  for (int i = 0; i < numargs; i++) {
    ...
    String message = "Processing argument " + String(i) + ": " + argument_name + " = " + argument_value;
    outputDebugln(message);
    ...
    if (argument_name =="pattern") {
      ...
      if (argument_value.length() == NUM_PIXELS) {
        ...
        timingPattern = 0;
        msPerUnit = 100;  // fixed value regardless of speed setting
...
void checkButton() {
  if (digitalRead(BUTTON_PIN) == LOW) {
    delay(50); // Debounce
    if (digitalRead(BUTTON_PIN) == LOW) {
      Serial.println("Button pressed. Opening Config Portal...");
      configurationPortal();
    }
  }
}
...
void configurationPortal() {
  int blinkCount;
  unsigned long temppreviousMillis;
  unsigned long tempcurrentMillis;

  Serial.println("Resetting saved preferences and WiFi Credentials...");
  ...
  const int tempStripBrightness = 255;
  const int tempColorLevel = 16;
  strip.setBrightness(tempStripBrightness);
  strip.clear();
  strip.setPixelColor(0, tempColorLevel, 0, 0);
  strip.show(); 


  // Setup Custom Parameter (gwidPin)
  WiFiManagerParameter custom_gwidPin_param("gwidPin", "GWID PIN (optional)", gwidPin, 6);;
  ...
  if (!wifiManager.startConfigPortal("GWID_AP")) {
    ...
    for (int blinkCount = 1; blinkCount < 9; blinkCount++) {
      tempcurrentMillis = millis();
      if (tempcurrentMillis - temppreviousMillis >= 500) {
        temppreviousMillis = tempcurrentMillis;
        strip.setPixelColor(RING_MIDPOINT, tempColorLevel * (blinkCount % 2), 0, 0);
        strip.show();
      }
    }
    // clear settings in case any SSID was saved with password even if no connection
    strip.clear();
    strip.show();
    wifiManager.resetSettings();
    WiFi.disconnect();
    Serial.println ("Not Configured. Rebooting.");
    ESP.restart();
  }

  Serial.println("Control Returned from GWID_AP access portal.");

  // Retrieve values from access portal custom parameters
  strcpy(gwidPin, custom_gwidPin_param.getValue());


  // Save custom preferences to flash
  preferences.begin("savedURL_config", false);
  // Check if user input a value for gwidPin, then update saved preferences accordingly
  ...
  // Any saved IP / port info was already cleared at the start of this function
  ...
  strip.setPixelColor(0, 0, tempColorLevel, 0);
  temppreviousMillis = millis();
  for (int blinkCount = 1; blinkCount < 9; blinkCount++) {
    tempcurrentMillis = millis();
    if (tempcurrentMillis - temppreviousMillis >= 500) {
      temppreviousMillis = tempcurrentMillis;
      strip.setPixelColor(RING_MIDPOINT, 0, tempColorLevel * (blinkCount % 2), 0);
      strip.show(); 
    }
  }
